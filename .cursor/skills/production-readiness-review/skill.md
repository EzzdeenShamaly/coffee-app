...
- A network failure surfaces a retry-able error state (per
  `flutter-bloc-gen`'s catch-and-emit-failure pattern), not a silent hang
...
| Offline handling | ❌ FAIL | order_history_screen.dart has no error state — the `OrderHistoryLoadFailure` variant is unhandled |
