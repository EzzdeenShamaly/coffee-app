...
`flutter-widget-gen`/`flutter-bloc-gen` or a manual fix for that).
...
|---|---|---|---|
| High | lib/features/cart/presentation/cart_screen.dart:34 | Widget calls `CartRepository().checkout()` directly in `build()` | Move to a `CheckoutSubmitted` handler on `CheckoutBloc`; widget calls `context.read<CheckoutBloc>().add(const CheckoutSubmitted())` |
...
compound). **Medium** = misplaced `BlocProvider` scope. **Low** = naming/
