---
name: flutter-bloc-gen
description: "Scaffolds a flutter_bloc Bloc with its sealed Equatable event and state hierarchies, mapping typed AppException failures to explicit failure states and depending on an abstract repository interface. Use when asked to add state management, a bloc, or event handling. Invoked as /flutter-bloc-gen."
---

# flutter-bloc-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/flutter-bloc-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/flutter-bloc-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** flutter-bloc-gen - [one-line description].`
