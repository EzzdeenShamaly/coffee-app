...
description: "Generates the data layer for a resource: an abstract repository interface, its implementation with typed error mapping, and the RepositoryProvider binding, so the application layer only depends on the interface. Use when asked to add a repository, API client, or data source. Invoked as /flutter-repository-gen."
---
